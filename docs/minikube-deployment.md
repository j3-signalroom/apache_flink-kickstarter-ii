...
This installs `docker`, `kubectl`, `minikube`, `helm`, and `envsubst` via Homebrew. Once complete, **launch Docker Desktop** before proceeding.
...
> Required: `docker`, `kubectl`, `minikube`, `helm`, and `envsubst`.
...
```bash
make flink-ui       # http://localhost:8081 (foreground — Ctrl+C to stop)
make flink-ui-bg    # http://localhost:8081 (background — returns prompt)
...
| `flink-ui` | Port-forward Flink UI and open `http://localhost:8081` (foreground) |
| `flink-ui-bg` | Port-forward Flink UI in the background and return the prompt |
