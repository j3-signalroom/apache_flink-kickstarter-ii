...
#
# *** Script Syntax ***
# ./deploy-cp-ptf-udf-timer-driven.sh <create | destroy> [--namespace=confluent]
...
TIME_JAR_PATH="$PROJECT_DIR/examples/ptf_udf_timer_driven/java/app/build/libs/app-1.0.0-SNAPSHOT.jar"
...
copy_udf_jar_to_flink_pods() {
    ...
    if [ ! -f "$TIME_JAR_PATH" ]; then
        print_error "Timer-driven JAR not found at: ${TIME_JAR_PATH}"
        print_error "Run 'make build-ptf-udf-timer-driven' first."
        exit 1
    fi
...
# ===========================================================================
do_create() {
    print_info "Deploying timer-driven PTF UDF via Flink SQL..."
    ...
    run_sql "Deploy timer-driven PTF UDF pipeline (SessionTimeoutDetector)" \
        "-- ═══════════════════════════════════════════════════════════════════
...
# ===========================================================================
do_destroy() {
    print_info "Tearing down timer-driven PTF UDF via Flink SQL..."
    print_info "  Namespace:     ${NAMESPACE}"
    print_info "  Flink cluster: ${FLINK_CLUSTER_NAME}"
