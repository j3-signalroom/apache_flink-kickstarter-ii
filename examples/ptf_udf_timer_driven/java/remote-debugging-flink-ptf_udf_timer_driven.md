...
Debugging a timer-driven PTF has a few unique considerations compared to a purely row-driven one:
...
- **TaskManager must be running** ─ the TaskManager pod only exists while a Flink job is active. Deploy your UDF first (`make deploy-cp-ptf-udf-timer-driven`) before attaching the debugger
