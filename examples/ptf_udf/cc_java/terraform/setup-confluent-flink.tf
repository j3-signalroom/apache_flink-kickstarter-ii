...
resource "confluent_flink_artifact" "ptf_udf_cc_java" {
  ...
  cloud            = local.cloud
  region           = local.aws_region
  artifact_file    = "${path.module}/examples/ptf_udf/cc_java/app/build/libs/apache_flink-kickstarter-ii-1.0.0-SNAPSHOT.jar"
...
resource "confluent_flink_statement" "create_udf" {
  statement = <<-EOT
    CREATE FUNCTION IF NOT EXISTS is_smaller
