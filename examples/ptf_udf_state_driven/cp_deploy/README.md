...
> This example deploys the **User Event Enricher** PTF UDF (source in [examples/ptf_udf_state_driven/java/](../java/)) by submitting SQL statements through the **Flink SQL Client** running directly on the JobManager pod.
...
| Entry point | `make deploy-cp-ptf-udf-state-driven` | `make deploy-cc-ptf-udf-state-driven` |
...
The deploy script copies the fat JAR (built from [examples/ptf_udf_state_driven/java/](../java/)) to `/opt/flink/usrlib/user-event-enricher.jar` on every JobManager and TaskManager pod using `kubectl exec`. The `CREATE FUNCTION ... USING JAR` statement then references this pod-local path.
...
```bash
make deploy-cp-ptf-udf-state-driven
```
...
```bash
make teardown-cp-ptf-udf-state-driven
```
