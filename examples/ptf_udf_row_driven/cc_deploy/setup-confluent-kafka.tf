resource "confluent_kafka_cluster" "ptf_udf_row_driven" {
...
module "kafka_api_key_rotation" {
  ...
  resource = {
    id          = confluent_kafka_cluster.ptf_udf_row_driven.id
    api_version = confluent_kafka_cluster.ptf_udf_row_driven.api_version
    kind        = confluent_kafka_cluster.ptf_udf_row_driven.kind
...
resource "confluent_kafka_topic" "user_events" {
  ...
  rest_endpoint = confluent_kafka_cluster.ptf_udf_row_driven.rest_endpoint
  credentials {
...
resource "confluent_kafka_topic" "orders_expanded" {
  kafka_cluster {
    id = confluent_kafka_cluster.ptf_udf_row_driven.id
  }
  topic_name    = "orders_expanded"
  rest_endpoint = confluent_kafka_cluster.ptf_udf_row_driven.rest_endpoint
  credentials {
    key    = module.kafka_api_key_rotation.active_api_key.id
    secret = module.kafka_api_key_rotation.active_api_key.secret
