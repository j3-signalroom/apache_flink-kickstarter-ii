# Remote Debugging a Row-Driven Flink PTF UDF
...
> JDWP is already enabled on the TaskManager, debug configurations for both VS Code and IntelliJ IDEA are in place, and port-forwarding is automated as a pre-launch task. Once your cluster is running and your UDF is deployed, use the **"Attach to Flink TaskManager (Row-Driven)"** debug configuration to attach to the remote JVM and hit your breakpoints.
>
...
3. Select the **"Attach to Flink TaskManager (Row-Driven)"** debug configuration in your IDE:
...
The `projectName` is `"app"` because that is the Gradle subproject name defined in `examples/ptf_udf_row_driven/java/settings.gradle.kts`. This tells the debugger which classpath and source roots to use for resolving breakpoints.
...
- **TaskManager must be running** — the TaskManager pod only exists while a Flink job is active. Deploy your UDF first (`make deploy-cp-ptf-udf-row-driven`) before attaching the debugger
