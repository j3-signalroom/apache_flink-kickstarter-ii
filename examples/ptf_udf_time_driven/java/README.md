# Java Process Table Function (PTF) User-Defined Function (UDF) type ─ Session Timeout Detector, a time-driven PTF example

> The Session Timeout Detector is driven by **event-time timers** that fire when a user becomes inactive.
>
> Unlike the companion [User Event Enricher](../../ptf_udf_state_driven/java/) (which is purely state-driven — state transitions triggered only by incoming rows), this PTF uses Flink's timer service to schedule future actions and react when those timers fire.
>
> This example demonstrates how the Process Table Function (PTF) API in Flink 2.1+ enables building fully stateful, **time-driven** operators in Java that are directly callable from SQL.

**Table of Contents**
<!-- toc -->
+ [**1.0 Timers and time-driven processing in a Process Table Function**](#10-timers-and-time-driven-processing-in-a-process-table-function)
...
+ [**3.0 State-driven vs time-driven ─ comparing the two PTF examples**](#30-state-driven-vs-time-driven--comparing-the-two-ptf-examples)
+ [**4.0 Resources**](#40-resources)
<!-- tocstop -->

## **1.0 Timers and time-driven processing in a Process Table Function**

A **Process Table Function (PTF)** is a stateful operator that sits inside the Flink dataflow graph. The [companion example](../../ptf_udf_state_driven/java/) showed how a PTF can maintain per-user state and transition that state purely in response to incoming rows. This example adds a second dimension: **time**.
...
Kafka (user_activity)
  ...
  SessionTimeoutDetector(
      ...
      on_time => DESCRIPTOR(event_time)
         ...
         │
         │  Per-user time-driven detection:
...
## **3.0 State-driven vs time-driven ─ comparing the two PTF examples**

| Aspect | [User Event Enricher](../../ptf_udf_state_driven/java/) (state-driven) | **Session Timeout Detector** (time-driven) |
|---|---|---|
...
| State transitions | Row-driven only (login → new session) | Row-driven (update count, reset timer) + time-driven (timeout → clear state) |
