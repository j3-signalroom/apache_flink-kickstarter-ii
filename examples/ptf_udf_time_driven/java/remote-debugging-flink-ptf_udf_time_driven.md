...
>
> - **VS Code:** Open the **Run and Debug** panel (Shift+Cmd+D), select **"Attach to Flink TaskManager"** from the dropdown, then press **F5**.
> - **IntelliJ IDEA:** Open the **Run/Debug Configurations** dropdown (top-right toolbar), select **"Attach to Flink TaskManager"**, then click **Debug** (Ctrl+D / Shift+F9).
...
Both IDEs ship pre-configured **"Attach to Flink TaskManager"** configurations. Each attach configuration wires the port-forward script as a pre-launch step so the entire flow is one click.
