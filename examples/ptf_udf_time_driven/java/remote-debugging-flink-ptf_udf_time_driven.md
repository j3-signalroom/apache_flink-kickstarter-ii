# Remote Debugging a Time-Driven Flink PTF UDF
...
> JDWP is already enabled on the TaskManager, debug configurations for both VS Code and IntelliJ IDEA are in place, and port-forwarding is automated as a pre-launch task. Once your cluster is running and your UDF is deployed, use the **"Attach to Flink TaskManager (Time-Driven)"** debug configuration to attach to the remote JVM and hit your breakpoints.
>
> **Note:** Make sure you select the **"Attach to Flink TaskManager (Time-Driven)"** configuration before launching the debugger. Starting a *launch* (rather than *attach*) configuration will attempt to run a local Java process, which fails because UDFs have no `main()` method.
>
> - **VS Code:** Open the **Run and Debug** panel (Shift+Cmd+D), select **"Attach to Flink TaskManager (Time-Driven)"** from the dropdown, then press **F5**.
> - **IntelliJ IDEA:** Open the **Run/Debug Configurations** dropdown (top-right toolbar), select **"Attach to Flink TaskManager (Time-Driven)"**, then click **Debug** (Ctrl+D / Shift+F9).
...
1. **Deploy** the full stack and your UDF:
    ...
    make deploy-cp-ptf-udf-time-driven  # Build UDF JAR, copy to Flink pods, submit SQL
    ```

2. **Set a breakpoint** in your UDF ─ open `SessionTimeoutDetector.java` and click in the gutter at the first line of the `eval()` method:
...
3. Select the **"Attach to Flink TaskManager (Time-Driven)"** debug configuration in your IDE:
...
4. **Start the debugger** ─ your IDE will port-forward to the TaskManager pod automatically, then attach:
    ...
    - **VS Code:** Press **F5**
    - **IntelliJ IDEA:** Click **Debug** (Ctrl+D / Shift+F9)
...
6. **Debug** ─ your IDE pauses at your breakpoint. Inspect `input`, `state`, and local variables, step through the timer registration logic, and watch `state.eventCount` and `state.lastEventType` update as you step over lines.
...
Both IDEs ship a pre-configured **"Attach to Flink TaskManager (Time-Driven)"** remote debug configuration. The attach configuration wires the port-forward script as a pre-launch step so the entire flow is one click.
...
{
  "type": "java",
  "name": "Attach to Flink TaskManager (Time-Driven)",
  "request": "attach",
  "hostName": "localhost",
  "port": 5005,
  "projectName": "app",
...
| **Attach to Flink TaskManager (Time-Driven)** | Remote JVM Debug config that attaches to `localhost:5005`. The port-forward config runs automatically as a "Before launch" task |

## **3.0 Debugging Timer-Driven PTFs**

Debugging a time-driven PTF has a few unique considerations compared to a purely state-driven one:

- **Timer fire timing:** Timers fire when the watermark advances past the timer's registered time. In a debug session with paused breakpoints, watermarks don't advance, so timers won't fire while you're stepping through `eval()`. To trigger an `onTimer()` breakpoint, you need to resume execution and let the watermark progress.
- **Named timer replacement:** If you're debugging the inactivity pattern, watch the `TimeContext.registerOnTime()` call ─ each call with the same name replaces the previous timer. Step over this line to see the timer being rescheduled.
- **State at timer fire:** When `onTimer()` fires, the state contains whatever was last written by `eval()`. Inspect `state.lastEventType` and `state.lastPayload` to see which event was the last one before the timeout.
- **`suspend=y` for timer debugging:** If you set `suspend=y` in the JDWP config, the JVM pauses before any processing. This lets you set breakpoints in both `eval()` and `onTimer()` before any events are processed.

## **4.0 Important Caveats**

- **TaskManager must be running** ─ the TaskManager pod only exists while a Flink job is active. Deploy your UDF first (`make deploy-cp-ptf-udf-time-driven`) before attaching the debugger
