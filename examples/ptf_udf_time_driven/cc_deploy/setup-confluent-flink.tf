...
# ═══════════════════════════════════════════════════════════════════════════════
# Shared: Upload the JAR as a Flink artifact (contains both PTF classes)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ── State-driven UDF registration and pipeline ────────────────────────────────

resource "confluent_flink_statement" "create_user_event_enricher" {
  statement = <<-EOT
...
resource "confluent_flink_statement" "insert_enriched_events" {
  ...
  depends_on = [
    confluent_flink_statement.create_user_event_enricher
  ]
}
...
resource "confluent_flink_statement" "create_session_timeout_detector" {
  ...
  depends_on = [
    confluent_flink_statement.insert_enriched_events
  ]
}
