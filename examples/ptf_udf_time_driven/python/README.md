# Why is there no Community or Confluent Python support for time-driven PTF UDFs?
...
The [companion state-driven example](../../ptf_udf_state_driven/python/) explains the general reasons why PTFs are Java-only. Time-driven PTFs add an additional layer of complexity:
...
These are *additional* to the state access, changelog, and set-semantic limitations described in the [state-driven Python README](../../ptf_udf_state_driven/python/).

---

## **2.0 Practical workarounds**

The same workarounds from the [state-driven example](../../ptf_udf_state_driven/python/) apply here, with one key note: **timer-based patterns are particularly well-suited to the DataStream API workaround** (Option 3), since PyFlink's `KeyedProcessFunction` does support timers.
...
PyFlink's `KeyedProcessFunction` supports both state and timers, making it the closest Python approximation to a time-driven PTF:
