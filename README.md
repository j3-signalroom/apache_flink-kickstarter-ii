...
| Type | Purpose | Confluent Platform on Minikube | Confluent Cloud |
| --- | --- | --- | --- |
| [Scalar UDF-type](examples/scalar_udf/java/README.md) | Walks through both **local** and cloud environments *building*, *deploying*, and *testing* **seven Scalar UDFs** bundled in a single JAR that illustrate the simplest and oldest category of Flink user-defined function — a pure, row-at-a-time function callable from SQL. The seven UDFs span three themes. The **unit-conversion primer** (the "Hello, UDF" pair) is **`CelsiusToFahrenheit`** and **`FahrenheitToCelsius`** ─ plain `DOUBLE → DOUBLE` arithmetic, the smallest possible shape of a scalar UDF. The **PII & data-governance** trio is **`PseudonymizePii`** (keyed HMAC-SHA256 pseudonym with a namespace argument for domain separation ─ irreversibly hides PII from analysts while keeping joins and `GROUP BY` stable), **`ResolveIdentity`** (un-keyed SHA-256 of a per-type normalized identity signal ─ a portable canonical join key across systems that don't share a secret), and **`TokenizePan`** (tokenizes a PAN into a `{BIN}{HMAC-hex middle}{last-4}` shape, taking the Flink pipeline out of PCI DSS scope while preserving first-6 and last-4 for fraud analytics and reconciliation). The **pipeline-reliability** pair is **`DedupKey`** (length-prefix-encoded SHA-256 over an `ARRAY<STRING>` of business fields ─ a stable idempotency key that defangs at-least-once duplicates via `ROW_NUMBER() OVER (PARTITION BY dedup_key …)`) and **`ConsistentBucket`** (Kafka-compatible 32-bit `murmur2`, the same hash Kafka's default partitioner uses ─ so Flink-computed buckets match Kafka-producer partitions, database shards, and cache slots byte-for-byte). | <p style="text-align: center;">[`CP Deploy`](examples/scalar_udf/cp_deploy/README.md)</p> | <p style="text-align: center;">[`CC Deploy`](examples/scalar_udf/cc_deploy/README.md)</p> |
