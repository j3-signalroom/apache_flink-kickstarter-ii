...
+ [**3.0 Debugging the Examples**](#30-debugging-the-examples)
    - [**3.1 Apache Flink UDF Debugging with Java Debug Wire Protocol (JDWP)**](#31-apache-flink-udf-debugging-with-java-debug-wire-protocol-jdwp)
        ...
        + [**3.1.2 Scalars**](#312-scalars)
            ...
            - [**3.1.2.2 Debugging the Fahrenheit to Celsius Scalar UDF (`FahrenheitToCelsius`)**](#3122-debugging-the-fahrenheit-to-celsius-scalar-udf-fahrenheittocelsius)
...
| Type | Purpose | Confluent Platform on Minikube | Confluent Cloud |
| --- | --- | --- | --- |
| Scalar UDF-type [Java](examples/scalar_udf/java/README.md) [Python](examples/scalar_udf/python/README.md) [Python CC](examples/scalar_udf/python_cc/README.md) | Walks through both **local** and cloud environments *building*, *deploying*, and *testing* **eight Scalar UDFs** bundled in a single JAR that illustrate the simplest and oldest category of Flink user-defined function — a pure, row-at-a-time function callable from SQL. The eight UDFs span three themes. The **unit-conversion primer** (the "Hello, UDF" pair) is **`CelsiusToFahrenheit`** and **`FahrenheitToCelsius`** ─ plain `DOUBLE → DOUBLE` arithmetic, the smallest possible shape of a scalar UDF. The **PII & data-governance** quartet is **`PseudonymizePii`** (keyed HMAC-SHA256 pseudonym with a namespace argument for domain separation ─ irreversibly hides PII from analysts while keeping joins and `GROUP BY` stable), **`ResolveIdentity`** (un-keyed SHA-256 of a per-type normalized identity signal ─ a portable canonical join key across systems that don't share a secret), **`TokenizePan`** (tokenizes a PAN into a `{BIN}{HMAC-hex middle}{last-4}` shape, taking the Flink pipeline out of PCI DSS scope while preserving first-6 and last-4 for fraud analytics and reconciliation), and **`MaskPiiRegex`** (free-form-text PII masking using Microsoft Presidio's regex recognizers, with Java and Python implementations that produce byte-identical output for the same input — the Python side calls Presidio directly, the Java side ports its patterns and validation logic verbatim). The **pipeline-reliability** pair is **`DedupKey`** (length-prefix-encoded SHA-256 over an `ARRAY<STRING>` of business fields ─ a stable idempotency key that defangs at-least-once duplicates via `ROW_NUMBER() OVER (PARTITION BY dedup_key …)`) and **`ConsistentBucket`** (Kafka-compatible 32-bit `murmur2`, the same hash Kafka's default partitioner uses ─ so Flink-computed buckets match Kafka-producer partitions, database shards, and cache slots byte-for-byte). | <p style="text-align: center;">[`CP Deploy`](examples/scalar_udf/cp_deploy/README.md)</p> | <p style="text-align: center;">[`CC Deploy`](examples/scalar_udf/cc_deploy/README.md)</p> |
...
##### **3.1.2.2 Debugging the Fahrenheit to Celsius Scalar UDF (`FahrenheitToCelsius`)**

> For the full deep-dive, see [Remote Debugging Flink Scalar UDFs](examples/scalar_udf/java/remote-debugging-flink-scalar_udf.md) (§1.3).
>
> The `FahrenheitToCelsius` ships in the **same uber JAR** as `CelsiusToFahrenheit`, so the same `make deploy-cc-scalar-udf` command and the same **"Attach to Flink TaskManager (scalar udf)"** debug configuration are used. The deploy script registers both Scalars as separate Flink SQL functions and starts an `INSERT INTO fahrenheit_to_celsius (sensor_id, fahrenheit_temperature, celsius_temperature) SELECT sensor_id, fahrenheit_temperature, fahrenheit_to_celsius(fahrenheit_temperature) FROM fahrenheit_reading;` pipeline alongside the scalar udf examples job.
...
Use the **same** **"Attach to Flink TaskManager (scalar udf)"** configuration as `FahrenheitToCelsius` — both Scalars run in the same TaskManager pod from the same JAR. The IDE will [automatically port-forward](scripts/port-forward-taskmanager.sh) to the TaskManager pod and attach to the JDWP agent on port `5005`.
...
```bash
make produce-fahrenheit-reading-record
```

The sample record has two items in its comma-separated list, so `eval()` will run once and emit a Celsius value.
...
Your IDE will pause at your breakpoint. Inspect `input`, then return the calculated Celsius value.
